  ...
  const columns: ColumnDef<Expense>[] = [
    {
      ...
      header: 'Miqdori',
      cell: ({ row }) => (
        <>{formatNumberComma(row.getValue("amount"))}</>
      )
    },
    {
      accessorKey: "expense_category.name",
      header: () => (
        <span className="hidden md:table-cell">Kategoriya</span>
      ),
      cell: ({ row }) => {
        const category = categories?.data.map(c => ({ value: c.id, label: c.name })).find(
          (c) => c.value === row.original.expense_category?.id
        )
      ...
      cell: ({ row }) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem className="focus:bg-red-100 focus:text-red-800" onClick={() => setOpen(Number(row.getValue("id")))}>O'chirish</DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
      )
    }
